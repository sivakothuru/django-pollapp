...
def vote(request, poll_id):
    ...
    except (KeyError, Choice.DoesNotExist), e:
...
def polls_with_most_votes(request):
    ...
    for poll in polls:
        ...
        for choice in choices:
            choice_set[choice.choice_text] = Vote.objects.filter(
                                             choice=choice).count()
        fulldata[poll.question] = choice_set
    newA = dict(sorted(dic.iteritems(), key=operator.itemgetter(1),
                reverse=True)[:5])
    return render_to_response("polls/max_votes_polls.html",
                              {'fulldata': fulldata},
                               context_instance=RequestContext(request))
